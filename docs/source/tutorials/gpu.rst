...
.. topic:: Methods of Setting up CUDA and |project|
    ...
    2. :ref:`Compile pyrand from the source <gpu-compile-pyrand>` for a specific version of CUDA to use an existing CUDA library. In this way, you can keep the current CUDA installation.
...
First, make sure |project| recognizes the CUDA libraries and GPU device. There are a number of functions available in :ref:`pyrand.device <Device Inquiry>` module to inquiry GPU device.

Locate CUDA Toolkit
-------------------

Use :func:`pyrand.device.locate_cuda` function to find the location of CUDA home directory.
...
Use :func:`pyrand.device.get_nvidia_driver_version` function to make sure |project| can detect the NVIDIA driver.
...
Use :func:`pyrand.device.get_processor_name()` and :func:`pyrand.device.get_gpu_name()` to find the name of CPU and GPU devices, respectively.
...
The number of CPU threads and GPU devices can be obtained respectively by :func:`pyrand.device.get_num_cpu_threads` and :func:`pyrand.device.get_num_gpu_devices()` functions.
...
The :func:`pyrand.info` function also obtains general information about |project| configuration and devices.
...
First, create a sample Toeplitz matrix with ten million in size using :func:`pyrand.toeplitz` function.
...
Next, create an :class:`pyrand.Matrix` object from matrix `A`:
...
Compute the log-determinant of the above matrix on GPU by passing ``gpu=True`` to :func:`pyrand.logdet` function. Recall GPU can only be employed using `SLQ` method by passing ``method=slq`` argument.
...
.. literalinclude:: ../_static/data/pyrand.logdet.slq-verbose-gpu.txt
    :language: python
...
.. |docker-size| image:: https://img.shields.io/docker/image-size/sameli/pyrand
