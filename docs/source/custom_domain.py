...
Why this custom domain is needed:
    ...
    The interface function/class:
        pyrand.logdet.logdet(A, ...)
        pyrand.Interpolate.Interpolate(A, ...)

    The implementation functions/classes:
        pyrand.logdet._cholesky_method.cholesky_method(A, ...)
        pyrand.logdet._hutchinson_method.hutchinson_method(A, ...)
        pyrand.logdet._slq_method.slq_method(A, ...)

        pyrand.Interpolate._EXT_method.EXT_method(A, ...)
        pyrand.Interpolate._EIG_method.EIG_method(A, ...)
        pyrand.Interpolate._IMBF_method.IMBF_method(A, ...)
    ...
    function/class. If we do just as like in the above, like by adding
        ...
        .. autosummary::
            ...
            pyrand.logdet
            pyrand.logdet._slq_method.slq_method
    ...
    and 
        .. autosummary::
            ...
            pyrand.Interpolate
            pyrand.Interpolate._IMBF_method.IMBF_method

    then it creates a docstring with the signature

        pyrand.logdet._slq_method.slq_method(A, p, method='slq', lanczos_degr..)
        ... content of _slq_method

    and 

        pyrand.Interpolate._IMBF_method.IMBF_method(A, p, kind='IMBF', ti=...)
        ... content of _EXT_method

    but what we really want is

        pyrand.logdet(A, p, method='slq', lanczos_degr..)
        ... content of _slq_method

    and

        pyrand.Interpolate(A, p, kind='IMBF', ti=[], ...)
    ...
    (1) We want the NAME of the function in the docstring to be the name of
        ...
        name of the implementation function is
            ...
            pyrand.logdet._slq_method_slq_method
            pyrand.Interpolate._IMBF_method.IMBF_method
    ...
    (2) We want to take the signature of the implementation function, not the
        ...
        pyrand.logdet(A, p, method='slq', lanczos_degree=20)
        pyrand.Interpolate(A, p, kind='IMBF', ti=[])
...
How the custom domain solves this issue:
    ...
    .. custom-domain:function:: pyrand.logdet        <= interface function
       :impl: pyrand.logdet._slq_method.slq_method   <= implementation function
       :method: slq                                 <= value of 'method' arg

    and

    .. custom-domain:class:: pyrand.Interpolate           <= interface class
       :impl: pyrand.Interpolate._IMBF_method.IMBF_method <= implement... class
