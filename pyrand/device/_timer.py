...
class Timer(object):
    ...
    tic_initiated : bool, default=False
        Indicates whether the function :meth:`pyrand.Timer.tic` has been called
    ...
    wall_time : float, default=0
        ...
        updated by calling :meth:`pyrand.Timer.toc`.

    proc_time : float, default=0
        The CPU process time between a `tic` and a `toc` in seconds. This
        variable is updated by calling :meth:`pyrand.Timer.toc`.
    ...
    *current* process, and not other tasks that the process might perform
    ...
    :func:`pyrand.device.get_num_cpu_threads`). However, the process time can
    ...
    **Alternative Way of Measuring Time Using Function Returns:**
    ...
    :func:`pyrand.logdet` function, which returns the dictionary ``info``.
    ...
    by :class:`pyrand.Timer` as follows:
    ...
    **Accumulative Time Measure Using Hold:**
    ...
    def toc(self):
        """
        Measures time from the last call to :meth:`pyrand.Timer.tic`.
