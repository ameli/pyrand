...
class Memory(object):
    ...
    ini_mem_used : default=0
        The initial resident memory when :meth:`pyrand.Memory.start` is called.

    mem : int default=0
        The difference between the current resident memory when
        :meth:`pyrand.Memory.read` is called and the initial resident memory.
    ...
    * To measure the resident memory in the current Python process, call
      :meth:`pyrand.Memory.get_resident_memory` function. For this, the class
      :meth:`pyrand.Memory` does not needed to be instantiated.
    ...
    * To measure the *acquired* memory between two points of the code (that is,
      ...
      :class:`pyrand.Memory` class. Then call the two functions
      :meth:`pyrand.Memory.start` and :meth:`pyrand.Memory.read` of the
    ...
    :meth:`pyrand.Memory.read` in this example reads the *difference* between
    ...
    def read(self, human_readable=False):
        ...
        .. note::
            ...
            This method should be called after :py:meth:`pyrand.Memory.start` is
            called.
        ...
        pyrand.Memory.start
        pyrand.Memory.get_resident_memory

        Notes
        -----

        This method reads the *difference* between the resident memory from
        when :meth:`pyrand.Memory.start` is called to the point where this
        method is called. Hence, this method measures the *acquired* memory
        in between two points.

        In contrast, the function :meth:`pyrand.Memory.get_resident_memory`
    ...
    def get_resident_memory(human_readable=False):
        ...
        Note that, in contrast, :meth:`pyrand.Memory.read` reads the
        *difference* between the resident memory from when
        :meth:`pyrand.Memory.start` is called to the point where this method is
