...
def traceinv(
    ...
    :class:`pyrand.AffineMatrixFunction` with the parameter :math:`t`, then for
    ...
    A : numpy.ndarray, scipy.sparse, :class:`pyrand.Matrix`, or \
            :class:`pyrand.AffineMatrixFunction`
        A non-singular sparse or dense matrix or linear operator. The linear
        operators :class:`pyrand.Matrix` and :class:`pyrand.AffineMatrixFunction`
        can be used only if ``method=slq``. See details in
        :ref:`slq method <pyrand.traceinv.slq>`. If ``method=cholesky``, the
    ...
    p : float, default=1.0
        ...
        * If ``method=slq``, :math:`p` should be non-negative real number.
        ...
        .. note::
            ...
            If :math:`p < 0`, use :func:`pyrand.trace` function.
    ...
    method : {'eigenvalue', 'cholesky', 'hutchinson', 'slq'}, \
        ...
        * :ref:`eigenvalue <pyrand.traceinv.eigenvalue>`
        * :ref:`cholesky <pyrand.traceinv.cholesky>`
        * :ref:`hutchinson <pyrand.traceinv.hutchinson>`
        * :ref:`slq <pyrand.traceinv.slq>`
    ...
    traceinv : float or numpy.array
        ...
        :class:`pyrand.AffineMatrixFunction` with an array of ``parameters``,
    ...
    info : dict
        ...
        * ``matrix``:
            ...
            * ``num_inquiries``: `int`, The size of inquiries of each parameter
              ...
              `1`. For more details see :ref:`slq method <pyrand.traceinv.slq>`.
    ...
    * A linear operator representing a matrix using :class:`pyrand.Matrix` (
      only if ``method=slq``).
    * A linear operator representing a one-parameter family of an affine matrix
      ...
      :class:`pyrand.AffineMatrixFunction` (only if ``method=slq``).

    **Output:**

    The output is a scalar. However, if `A` is the linear operator of the type
    :class:`pyrand.AffineMatrixFunction` representing the matrix function
    ...
    **Sparse matrix:**
    ...
    :func:`pyrand.toeplitz` function.

    .. code-block:: python

        >>> # Import packages
        >>> from pyrand import toeplitz, traceinv
    ...
    **Large matrix:**
    ...
    :func:`pyrand.sample_matrices.toeplitz_traceinv` for details.

    .. code-block:: python

        >>> from pyrand.sample_matrices import toeplitz_traceinv
    ...
    **Matrix operator:**
    ...
    The following example uses an object of :class:`pyrand.Matrix`. Note that
    this can be only applied to ``method=slq``. See further details in
    :ref:`slq method <pyrand.traceinv.slq>`.

    .. code-block:: python
        :emphasize-lines: 8

        >>> # Import matrix operator
        >>> from pyrand import toeplitz, traceinv, Matrix
    ...
    **Affine matrix operator:**
    ...
    :class:`pyrand.AffineMatrixFunction` to create the linear operator:
    ...
    in :ref:`slq method <pyrand.traceinv.slq>`.

    .. code-block:: python
        :emphasize-lines: 8

        >>> # Import affine matrix function
        >>> from pyrand import toeplitz, traceinv, AffineMatrixFunction
