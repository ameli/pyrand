...
``/pyrand/Utilities/PlotUtilities.py`` handles this as follows:

* If the environment variable ``DISPLAY`` does not exist or is ``''``, or
* If the environment variable ``PYRAND_NO_DISPLAY`` is set to ``'True'``,
...
In the beginning of the above test files, and before importing ``pyrand``,
these lines should be added:

::

    import os
    os.environ['PYRAND_NO_DISPLAY'] = 'True'
...
``PYRAND_NO_DISPLAY``, the ``matplotlib`` in the module ``plot_utilities.py``
...
import os
os.environ['PYRAND_NO_DISPLAY'] = 'True'   # define before importing pyrand
