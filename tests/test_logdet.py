...
import numpy
import scipy.sparse
from pyrand.sample_matrices import toeplitz, toeplitz_logdet
...
def relative_error(estpyrand, exact):
    """
    Compute the relative error of an estpyrand, in percent.
    """

    tol = 1e-15
    if numpy.abs(exact) < tol:
        if numpy.abs(estpyrand - exact) < tol:
            relative_error = 0.0
        else:
            relative_error = numpy.inf
    else:
        relative_error = numpy.abs((estpyrand - exact) / exact) * 100.0
